...
- **[ ] Folder-level image gallery** — *(Medium LOE)* Store and browse rig/amp photos at the folder level. Storage: images sit in the actual folder on disk (`.jpg`, `.png`, `.webp`) — no sidecar needed, the folder IS the container. UI options discussed: (a) camera icon badge in FolderTree when images are present, click opens a lightbox/gallery overlay; (b) a collapsible image strip at the top of the FileList when a folder is selected. Recommended: right-click folder → "View Photos" opens a modal gallery; folder tree shows a small camera badge if any images exist. IPC: `folder:scanImages` returns image paths in a folder; `file:readImageBase64` returns image data for display. Main complexity is the gallery UI component and deciding whether images from subfolders are shown at the parent level. No write/edit needed in v1 — read/display only, user manages image files in Explorer/Finder.
...
- **[ ] Blank xlsx import template with lookup dropdowns** — *(Low-Medium LOE)* Right-click folder → "Download blank import template" generates an Excel file with correct column headers (same as the pre-filled export template) but no data rows. Sheet 2 contains lookup tables: Gear Type values, Tone Type values, etc. Sheet 1 cells in those columns use Excel data validation (dropdown lists) pointing to Sheet 2 ranges — same as a hand-crafted template. The `xlsx` library supports data validation via `ws['!dataValidation']` (or via `xlsx-js-style` / SheetJS Pro). Caveat: the free `xlsx` package (SheetJS community edition v0.18.x) has limited data validation support — may need to write the validation XML manually into the workbook XML. Worth prototyping to confirm the free library can produce working dropdowns before committing. If not, alternative is to embed a pre-built template file as a binary asset and just fill in the lookup sheet values at generation time.
