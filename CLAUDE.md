...
- **[ ] Manufacturer filter in list view** — Add a Manufacturer (gear_make) dropdown filter to the list view filter bar, alongside the existing Gear Type, Tone Type, Preset, and Name Contains controls. Populate options from distinct values in loaded files, same pattern as the gear/tone dropdowns.

- **[ ] Double-click grid column header to auto-size** — Double-clicking a column header resizes it to fit the widest cell value in that column (like Excel auto-fit). Iterate visible rows, measure text width via a canvas `measureText` call or a hidden DOM element, clamp to a min/max, and update the column width in state.

- **[ ] Drag-to-reorder grid columns** — Drag a column header to a new position to reorder columns. Same pattern as capture-lab. Persist order to localStorage alongside visibility settings. Use `draggable` on `<th>` with `dragover`/`drop` handlers; swap indices in the `visibleCols` array.

- **[ ] CSV export from grid not working** — Bug: CSV export from the grid view is broken. Needs investigation — likely the column/row mapping or the XLSX write path for CSV format.

- **[ ] Blank xlsx import template with lookup dropdowns (with sample)** — Update "Generate blank import template" to include a Sheet 2 with lookup tables for Gear Type and Tone Type valid values, and apply Excel data validation dropdowns to those columns on Sheet 1. Show a sample data row on Sheet 1 so users understand the expected format. Requires manual OOXML `dataValidation` injection since SheetJS v0.18.x has limited validation API support.
...
- **[ ] Per-column grid filters (multi-select + text search)** — Click a filter icon on any grid column header to open a popup with: (a) a text search input and (b) a checklist of all distinct values in that column (like Excel AutoFilter or DevExtreme dxDataGrid). Multiple column filters compose with AND logic. Active filter indicated by a highlighted icon on the header. This would subsume many of the one-off filter chips (gear type, tone type, preset, name contains) and apply consistently to any column. Medium-high LOE; consider a `columnFilters: Record<string, string[]>` state shape where an empty array means "no filter" for that column.
