...
- **[HIGH PRIORITY] Per-architecture batch settings**: allow one mixed queue submission to use different settings per selected architecture, such as `A2 = 500 epochs` and `Standard/A1 = 1000 epochs`, instead of forcing one shared epoch count across all selected architectures.
- **[HIGH PRIORITY] New training output folder detection**: when a run creates new output subfolders that did not exist yet, especially architecture split folders, the app should still surface the usual "new NAM folder/files created" notice and offer a quick jump to them instead of silently missing the newly created subfolder.
- **[HIGH PRIORITY] Queue visual ordering after restart**: when the queue is stopped mid-run (some jobs complete, some still queued) and the app is relaunched, `getPersistableTrainerQueueJobs()` separates unfinished jobs from terminal jobs and places unfinished first â€” so the visual order scrambles: completed rows appear at the top (from the previous run), then new completions fill in from the top again, leaving completed rows at both top and bottom with in-progress/queued sandwiched in the middle. Fix: preserve the original submission order when restoring the queue from disk; do not reorder by terminal/non-terminal status.
...
- Training drag-and-drop WAV intake: support dragging WAV files from Windows Explorer/Finder into the Training pageâ€™s WAV input area so the dropped files populate the selected capture list.
- Training history paging: the History tab currently renders every entry into a single grouped list. With 1000s of past runs the page becomes heavy. Add paging (or windowed/virtual scrolling) â€” default ~50â€“100 entries per page, with `Older â†’` / `â† Newer` controls and a `Show all` escape hatch. Filter / search must apply across the full history, not just the current page.
