...
- **Bug: drag-and-drop fails in the training queue.** Two different mechanisms exist in `TrainingPanel.tsx` and only one was fixed: batch-level reordering (~1988) was deliberately rewritten to a custom mousedown/mousemove/mouseup drag after a code comment records that native HTML5 DnD "does not reliably fire `drop` on same-window targets" in Electron on Windows - `onDragOver` fires but `onDrop` does not. Per-job reordering WITHIN a batch (~3374-3383) still uses that same native `draggable`/`onDragStart`/`onDragOver`/`onDrop` approach and was never migrated to the mouse-event pattern - this is the most likely source of "drag and drop fails". Confirm which specific drag (batch-level vs. job-level) is failing, then port job-level reordering to the same mouse-event-based approach as batch-level (`reorderTrainerJob` IPC call already exists and should not need to change).
...
- **[VERY SOON] Training queue semantics pass: reorder / park / sticky-batch behavior.** Current behavior is internally inconsistent and the list UI is misleading when a batch is already running.
  - `Run next` / move-to-top currently reorders the visible queue, but does **not** actually make that batch run next if another batch is active. Scheduler is sticky to the current `submissionId` (`pumpTrainerQueue(job.submissionId)` + `nextQueuedTrainerJob(preferSubmissionId)`), so after the active job finishes it keeps consuming the rest of the current batch until that batch has no queued items left. This contradicts the UI wording. Intended behavior for the next implementation: **do not interrupt the current job**, but if the user explicitly moves another batch to the top, clear the sticky preference after the current job and start the promoted batch next.
  - Batch reordering currently moves every non-active row in the submission block, not just queued rows. That means `success` / `error` / `canceled` rows, and potentially other non-queued rows, ride along during reorders and create odd split-batch visuals around the active row. Intended behavior: batch reorder operations should affect **queued jobs only**; the active/running job stays pinned, staged jobs stay in Staged Batches, and terminal rows should not participate in queue ordering.
  - Parking a batch halfway done currently stages only the remaining queued items. The active item keeps training, then the queue moves on to some other queued batch because no queued items remain in that submission. This is a reasonable default and should stay, but the UI/help text should make it explicit that **Park means "finish the current item, then freeze the rest of this batch"** - not "stop immediately."
  - Restart behavior is currently sensible and should be preserved: if the app is closed mid-run, any `starting` / `running` row is demoted back to `queued` on load and the queue is paused, effectively behaving like an emergency stop that requires explicit Resume.
  - Also review whether partially completed + parked batches should continue to keep their finished rows visible in the queue card, or whether history-only is cleaner once the unfinished remainder has moved to Staged Batches. Current pruning behavior may drop finished rows once all non-staged rows in the submission are `success`, even if the rest of the batch is now parked.
