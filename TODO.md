...
- **Simplify the folder tree right-click menu — currently cluttered and overwhelming.** User (2026-07-10): "very cluttered and overwhelming." Confirmed: `FolderTree.tsx`'s single-folder context menu (~line 872-1100+) renders roughly **24 distinct actions** across 4 unlabeled-until-you-look sections — Folder (Clean this folder, Save all, Revert all, Batch edit, Select all, New subfolder, Rename, Delete empty, Reveal in Explorer, Browse cards), Metadata (Generate template, Import metadata, Suggest metadata, Edit/Copy/Paste suggestion rules), Maintenance (Find duplicates, Training version report, Set/Change/Stop watch source), and Organize/Export (Export CSV/Excel, Create/Remove bundle, Remove Pack Info, Set folder color) — all fully expanded at once, every time, regardless of how often each one is actually used.
  - The sections themselves are a reasonable structure already (this isn't unstructured — it's just all-visible with no prioritization). A redesign should build on those 4 groupings rather than inventing new ones.
  - Directions worth considering: (a) a top tier of the ~4-6 truly common actions (Batch edit, Reveal in Explorer, Browse cards, Save/Revert) always visible, with the rest under a "More..." submenu or collapsed accordion per section; (b) hide entire sections when nothing in them applies (already partially done — sections use `{(a || b || c) && (...)}` guards) but go further and hide low-frequency items behind one extra click; (c) survey which of the 24 actions are actually used day-to-day before deciding what's "common" vs "buried", rather than guessing.
  - Multi-select folder menu (2+ folders selected) is already much shorter (~3 items: Compare selected folders, Export CSV, Export Excel) — worth understanding why that one already feels fine as a reference point for how sparse the single-folder menu could be.
